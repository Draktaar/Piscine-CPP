...
/*   Updated: 2025/07/29 16:59:10 by achu             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
ClapTrap::ClapTrap(void)
	: _name("Default"), _health(10), _energy(10), _atkDmg(0) {
...
ClapTrap::ClapTrap(std::string pName, int pHealth, int pEnergy, int pAtkDmg)
	: _name(pName), _health(pHealth), _energy(pEnergy), _atkDmg(pAtkDmg) {
	std::cout << "ClapTrap " << _name << " parameterized constructor called" << std::endl;
}
...
void	ClapTrap::attack(std::string const& target)
{
	if (_health <= 0)
	{
		std::cout << "Claptrap " << _name << " is out of service!" << std::endl;
		return ;
	}	
	else if (_energy <= 0)
	{
